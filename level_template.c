...
int level_template(){
	...
	//Put your level layout here
		...
		if((time(NULL) - npc_timer1) >=3){
			moveRight(NPC);
			npc_timer1 = time(NULL);
		}
		if ((time(NULL) - npc_timer2) >=6){
			moveLeft(NPC);
			npc_timer2 = time(NULL);
		}
